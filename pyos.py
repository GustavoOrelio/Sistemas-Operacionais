...
class os_t:
	...
	def syscall (self):
		...
		# Implementar aqui as outras chamadas de sistema
			...
			virtual_address = self.cpu.get_reg(1)
			physical_address = task.pddr_offset + virtual_address
			text = ""
			
			while True:
				valor_lido = self.memory.read(physical_address)
				if valor_lido == 0:
					break
				text = text + chr(valor_lido)
